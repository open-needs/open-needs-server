...
async def rest_update_need(
    need: NeedUpdateSchema,
    need_id: int,
    db: AsyncSession = Depends(get_db),
    ext: ONSExtension = Depends(get_extension),
):
    ...
    db_need = await update_need(ext, db, need_id=need_id, need=need_json)
    return db_need
