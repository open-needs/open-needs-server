...
@roles_router.get(
    "/", response_model=list[RoleReturnSchema], dependencies=[Depends(read_roles)]
)
async def rest_read_roles(
    skip: int = 0,
    limit: int = 100,
...
async def rest_create_role(
    role_name: str, role: RoleUpdateSchema, db: AsyncSession = Depends(get_db)
):
