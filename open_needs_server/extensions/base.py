...
class ONSExtension:
    ...
    def register_event(self, event: str, description: str):
        if event not in self.ons_app.ons_events:
            ...
            log.debug(
                f"Event already exist: {event} {self.name} (extension={extension})"
            )
    ...
    def register_listener(self, event: str, func: object, extra: dict = None):
        ...
        self.ons_app.ons_events[event]["listeners"].append(
            {"extension": self.name, "func": func, "extra": extra}
        )
    ...
    def fire_event(self, event, data):
        ...
        log.debug(
            f'Event "{event} fired for {len(self.ons_app.ons_events[event]["listeners"])} listeners'
        )

        # Calls all listeners in a fix sequence. No parallel execution
        for listener in self.ons_app.ons_events[event]["listeners"]:
            log.debug(
                f'Calling listener {listener["extension"]}:{listener["func"].__name__} for event {event}'
            )
            data = listener["func"](
                event, data=data, extra=listener["extra"], ext=self.name
            )
