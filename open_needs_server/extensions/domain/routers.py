...
async def rest_create_domain(
    ...
    if db_domain:
        raise HTTPException(
            status_code=400, detail="A domain with this title is already registered"
        )
...
async def rest_update_domain(
    ...
    if db_domain is None:
        raise HTTPException(status_code=404, detail="Project not found")
    db_domain = await update_domain(ext, db, domain_id=domain_id, domain=domain_json)
    return db_domain
