...
async def rest_read_organizations(
    skip: int = 0,
    limit: int = 100,
...
async def rest_create_organization(
    organization: OrganizationCreateSchema,
    db: AsyncSession = Depends(get_db),
    ext: ONSExtension = Depends(get_extension),
):
    organization_json = jsonable_encoder(organization)
    db_organization = await get_organization_by_title(
        ext, db, organization_title=organization.title
    )
    if db_organization:
...
async def rest_read_organization(
    organization_id: int,
    db: AsyncSession = Depends(get_db),
    ext: ONSExtension = Depends(get_extension),
):
...
async def rest_update_organization(
    organization: OrganizationCreateSchema,
    organization_id: int,
    db: AsyncSession = Depends(get_db),
    ext: ONSExtension = Depends(get_extension),
    ...
    db_organization = await update_organization(
        ext, db, organization_id=organization_id, organization=organization_json
    )
    return db_organization
...
async def rest_delete_organization(
    organization_id: int,
    db: AsyncSession = Depends(get_db),
    ext: ONSExtension = Depends(get_extension),
):
    ...
    try:
        db_organization = await delete_organization(
            ext, db, organization_id=organization_id
        )
