...
async def read_items(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    needs = await api.get_needs(db, skip=skip, limit=limit)
    return needs


@needs.post("/", response_model=schemas.Need)
async def create_need(need: schemas.NeedCreate, db: Session = Depends(get_db)):
    need_json = jsonable_encoder(need)

    try:
        need_db = await api.create_need(db=db, need=need_json)
